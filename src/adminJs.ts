...
async function renderAdminRoster(el) {
  ...
  el.innerHTML = \`
    ...
    <tbody>\${rows.map(u => \`<tr style="\${u.suspended_at ? 'opacity:.55;' : ''}"><td>\${avatarHtml(u, 24)}</td><td>\${esc(u.name)}\${u.suspended_at ? '<div style="font-size:10px;color:var(--danger);font-weight:700;margin-top:2px;" title="\${u.suspended_reason ? esc(u.suspended_reason) : ''}">SUSPENDED\${u.suspended_reason ? ' · ' + esc(u.suspended_reason) : ''}</div>' : ''}</td><td class="pin-display">\${u.pin}</td><td>\${statusBadge(u.role)}</td>
      ...
      <td style="display:flex;gap:6px;flex-wrap:wrap;"><select onchange="changeRole(\${u.id}, this.value)" style="width:auto;padding:6px 8px;font-size:11px;"><option value="">Change role…</option><option value="caller">Caller</option><option value="manager">Manager</option><option value="finisher">Finisher</option><option value="admin">Admin</option></select><button class="btn btn-ghost btn-sm" onclick="viewClockHistory(\${u.id},'\${esc(u.name)}')">History</button>\${u.suspended_at ? '<button class="btn btn-gold btn-sm" onclick="unsuspendUser(' + u.id + ')">Reinstate</button>' : '<button class="btn btn-ghost btn-sm" onclick="suspendUser(' + u.id + ')" style="color:var(--danger);border-color:rgba(239,68,68,.3);">Suspend</button>'}<button class="btn btn-danger btn-sm" onclick="removeUser(\${u.id})">Remove</button></td></tr>\`).join('')}</tbody></table></div></div>
